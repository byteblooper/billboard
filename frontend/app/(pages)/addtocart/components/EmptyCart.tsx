  ...
  return (
    <div className="max-w-md mx-auto px-4 sm:px-0">
      <div className="bg-white rounded-xl sm:rounded-2xl border border-violet-200 p-6 sm:p-12 text-center shadow-sm">
        <div className="w-16 h-16 sm:w-24 sm:h-24 mx-auto mb-4 sm:mb-6 rounded-full bg-linear-to-br from-violet-100 to-indigo-100 flex items-center justify-center">
          <ShoppingCart className="w-8 h-8 sm:w-12 sm:h-12 text-violet-400" />
        </div>
        <h3 className="text-xl sm:text-2xl font-bold text-violet-900 mb-2">Your Cart is Empty</h3>
        ...
        <div className="mt-6 sm:mt-8 pt-4 sm:pt-6 border-t border-violet-100">
          <p className="text-[10px] sm:text-xs text-violet-500 mb-2 sm:mb-3">Or check out</p>
          <div className="flex justify-center gap-3 sm:gap-4">
            <Link href="/map" className="text-xs sm:text-sm text-violet-600 hover:text-violet-800 font-medium flex items-center gap-1">
              <MapPin className="w-3 h-3 sm:w-4 sm:h-4" />
              Nearby Stores
            </Link>
            <Link href="/wilshlist" className="text-xs sm:text-sm text-violet-600 hover:text-violet-800 font-medium flex items-center gap-1">
      ...
      <div className="mt-6 sm:mt-8">
        <h3 className="text-base sm:text-lg font-bold text-violet-900 mb-3 sm:mb-4 text-center">Popular Products</h3>
        <div className="grid grid-cols-2 gap-2 sm:gap-3">
          {recommendedProducts.slice(0, 4).map((product) => (
            <Link
              key={product.id}
              href={`/productDetails/${product.id}`}
              className="group bg-white rounded-lg sm:rounded-xl border border-violet-200 p-2 sm:p-3 hover:shadow-lg transition-all duration-300"
            >
              <div className="aspect-square rounded-md sm:rounded-lg overflow-hidden mb-1.5 sm:mb-2 bg-violet-50">
                <Image
                  src={product.image}
                  alt={product.name}
                  width={150}
